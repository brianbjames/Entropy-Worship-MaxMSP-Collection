...
/*
	...
	grid variation of springoid example
...
function gridsize(v)
{
	...
	for (i=0;i<mygridsize;i++)
	{
		...
		for (j=0;j<mygridsize;j++)
		{
			...
			x = Math.random()*0.02+i/(mygridsize-1);
			y = Math.random()*0.02+j/(mygridsize-1);
			vx = (Math.random()-0.5)*0.001;
			vy = (Math.random()-0.5)*0.001;
			
			// create a new agent object
			agents[i][j] = new agent(i,j,x,y,vx,vy);	
...
function bang()
{
	...
	for (i=0;i<mygridsize;i++) {
		for (j=0;j<mygridsize;j++) {
			// right
			if (i<(mygridsize-1)) {
				outlet(1,agents[i][j].x,agents[i][j].y,agents[i+1][j].x,agents[i+1][j].y);
			}
			//bottom
			if (j<(mygridsize-1)) {
				outlet(1,agents[i][j].x,agents[i][j].y,agents[i][j+1].x,agents[i][j+1].y);
			}
		}
	}
	...
	for (i=0;i<mygridsize;i++) {
		for (j=0;j<mygridsize;j++) {
			outlet(0,agents[i][j].x,agents[i][j].y,agents[i][j].vx,agents[i][j].vy);
		}
	}
}
