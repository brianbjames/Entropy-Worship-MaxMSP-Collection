...
var linecolor = [0.4, 0.4, 0.4, 0.5];
var lineovercolor = [0.1, 0.1, 0.1, 1.];
...
function paint()
{
	...
	for (var i in connections) {
		...
		if (inputs_obj_idx != -1 && outputs_obj_idx != -1) {
			...
			if (connections[i].isSelected || connections[i].src == mouseOver.name || connections[i].dst == mouseOver.name) {
				if (mouseOverOption)
