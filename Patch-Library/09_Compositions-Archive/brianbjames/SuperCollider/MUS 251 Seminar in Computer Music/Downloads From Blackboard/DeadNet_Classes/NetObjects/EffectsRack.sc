...
EffectsRack {
	...
	*new {arg groupID, inBus=0, outBus=0, numSlots=5, effectsList;
	...
	gui { arg win, left, top;
		...
		rack = SCCompositeView(window, Rect(left,top, 170, 65+(20*numSlots)))
		...
		numSlots.do({arg i;
			var  group, button, buttonIndex, synthName, synth, slotGroup;
			slotGroup = slotGroups[i].nodeID;
			...
			button = 	SCPopUpMenu(rack, Rect(0, 0, 130, 15))
				...
				.action_({arg view;
					var synthName;
						slotGroup.postln;
						synthName = effectsList.at(view.value);
						
						ServerManager.servers.keys.do({ arg key;
							if (key != \internal)
								{
									ServerManager.servers[key].sendMsg(\g_freeAll, slotGroup);
									ServerManager.servers[key].sendMsg(\s_new, synthName, -1, 1, slotGroup, \in, inBus, \out, outBus);
								}
						});
						
					if (netType != \receiver) {	
						if(autoGuis[i].notNil){autoGuis[i].window.close};  //close open autoGui
						if (synthName.asSymbol != \bypass)
							{autoGuis.put(i, AutoGui(synthName, slotGroup))};
					}
				});
		});
